...
function getCompNameBySrc (file) {
   ...
   * */
  let fileHashName
  if (!compDependencies.getHash(file)) {
      fileHashName = `${getNameByFile(file)}$${hash(file)}`
      compDependencies.setHash(file, fileHashName)
  }
  return cache[file] || (cache[file] = fileHashName)
}
...
// 插件的未完成, 这儿应该按照 main.js 导出的 config 来进行 isApp isPage 识别，暂时不改，下次大版本升级 loader 的时候改
// 计算目标输出的路径等信息
